...
def final_training_with_best_params(student_choice, teacher_path, device, 
    ...
    for epoch in progress_bar:
        ...
        if val_accuracy > best_val_acc:
            ...
            save_path = f"results/distilled_{student_choice}_{shared_args.dataset}_best"
            # --- FIN MODIFICATION ---
...
def save_results(student_choice, all_results, best_params, best_accuracy, dataset_name): # MODIFIÉ: Accepte dataset_name
    ...
    output_dir = f'results/{student_choice}_stats'
    os.makedirs(output_dir, exist_ok=True)

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    
    # MODIFIÉ: Nom de fichier inclut le dataset
    results_file = f'{output_dir}/hypersearch_{student_choice}_{dataset_name}_{timestamp}.csv'
    ...
    # MODIFIÉ: Nom de fichier inclut le dataset
    params_file = f'{output_dir}/best_params_{student_choice}_{dataset_name}_{timestamp}.json'
...
def main():
    ...
    parser.add_argument('--teacher_path', type=str, default='results/fine_tuned_roberta_large_imdb', 
